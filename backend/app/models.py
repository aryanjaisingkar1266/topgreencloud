...
class CarbonAnalysis(Timestamps, Base):
    ...
    provider_id: Mapped[int | None] = mapped_column(ForeignKey("cloud_providers.id", ondelete="SET NULL"), index=True)
    provider_slug: Mapped[str | None] = mapped_column(String(120))
    total_co2e: Mapped[Decimal | None] = mapped_column(Numeric(52, 18))
    methodology_version: Mapped[str] = mapped_column(String(120))
    complete: Mapped[bool] = mapped_column(Boolean, server_default="false")
...
class CarbonAnalysisItem(Base):
    ...
    provider_slug: Mapped[str | None] = mapped_column(String(1000))
    service_name: Mapped[str | None] = mapped_column(String(1000))
    service_category: Mapped[str | None] = mapped_column(String(1000))
    region: Mapped[str | None] = mapped_column(String(1000))
    usage_quantity: Mapped[Decimal | None] = mapped_column(Numeric(24, 9))
    usage_unit: Mapped[str | None] = mapped_column(String(1000))
    estimated_co2e: Mapped[Decimal | None] = mapped_column(Numeric(48, 18))
    kg_co2e_per_unit: Mapped[Decimal | None] = mapped_column(Numeric(24, 9))
    source_name: Mapped[str | None] = mapped_column(String(1000))
    source_url: Mapped[str | None] = mapped_column(Text)
    source_methodology_version: Mapped[str | None] = mapped_column(String(1000))
    source_date: Mapped[date | None] = mapped_column(Date)
